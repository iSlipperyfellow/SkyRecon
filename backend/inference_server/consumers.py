...
class InferenceConsumer(AsyncWebsocketConsumer):
        ...
        if InferenceConsumer.inference_engine is None:
            ...
            detector_path = os.path.abspath(
                os.path.join(
                    ...
                    "../../runs/detect/skyrecon_detector7/weights/best.pt",
                )
            )
            classifier_path = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "../../ai/classifier.pth")
            )
    ...
    async def receive(self, text_data):
        ...
        if InferenceConsumer.inference_engine:
            ...
            await self.send(
                text_data=json.dumps({"status": "success", "detections": results})
            )

            # BROADCAST to the 'live_feed' group (dashboard)
            await self.channel_layer.group_send(
                "live_feed",
                {"type": "broadcast_frame", "frame": frame_b64, "detections": results},
            )
        else:
            await self.send(
                text_data=json.dumps(
                    {"status": "error", "message": "Inference engine not initialized"}
                )
            )
