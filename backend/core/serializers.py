...
from core.models import (
    Drone, Flight, Detection, Hazard, Telemetry, Runway,
    InferenceJob, NavigationPlan, AIModel
)
...
class DroneSerializer(GeoFeatureModelSerializer):
    ...
    class Meta:
        model = Drone
        fields = ['id', 'identifier', 'model', 'max_payload', 'home_location', 
                  'status', 'last_seen', 'recent_telemetry', 'created_at', 'updated_at']
...
class TelemetrySerializer(GeoFeatureModelSerializer):
    ...
    class Meta:
        model = Telemetry
        fields = ['id', 'drone', 'flight', 'timestamp', 'location', 'altitude', 
                  'velocity', 'heading', 'battery', 'created_at']
        geo_field = 'location'
        read_only_fields = ['id', 'created_at']
...
class FlightSerializer(GeoFeatureModelSerializer):
    ...
    class Meta:
        model = Flight
        fields = ['id', 'drone', 'drone_identifier', 'mission_name', 'start_time', 
                  'end_time', 'path', 'status', 'altitude_m', 'duration_seconds', 
...
class DetectionSerializer(GeoFeatureModelSerializer):
    ...
    class Meta:
        model = Detection
        fields = ['id', 'drone', 'drone_identifier', 'flight', 'timestamp', 'geometry', 
                  'bbox', 'label', 'confidence', 'image_url', 'hazard_score', 
                  'hazard_level', 'metadata', 'created_at']
        geo_field = 'geometry'
...
class HazardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Hazard
        fields = ['id', 'detection', 'score', 'level', 'acknowledged', 
                  'acknowledged_by', 'acknowledged_by_name', 'acknowledged_at', 
                  'archived', 'reasoning', 'created_at', 'updated_at']
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class RunwaySerializer(GeoFeatureModelSerializer):
    ...
    class Meta:
        model = Runway
        fields = ['id', 'name', 'airport_code', 'geometry', 'centerline', 
                  'length_m', 'width_m', 'active', 'created_at', 'updated_at']
        geo_field = 'geometry'
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class InferenceJobSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = InferenceJob
        fields = ['id', 'drone', 'drone_identifier', 'flight', 'status', 'frame_url',
                  'submitted_at', 'completed_at', 'detections_count', 'error_message',
                  'external_job_id']
...
class NavigationPlanSerializer(GeoFeatureModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'drone', 'drone_identifier', 'flight', 'planned_path',
                  'constraints', 'optimization_metric', 'created_at', 'updated_at']
...
class AIModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AIModel
        fields = ['id', 'name', 'version', 'framework', 'status', 'accuracy', 
...
class DroneCommandSerializer(serializers.Serializer):
    ...
    route = serializers.JSONField(required=False, help_text="For REROUTE: {waypoints: [{lat, lng}, ...]}")
