...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        model_files = [f for f in os.listdir(backend_dir) if f.endswith(('.pt', '.onnx'))]
        
        if not model_files:
            self.stdout.write(self.style.WARNING(f'No model files found in {backend_dir}'))
            return
        ...
        for file_name in model_files:
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully imported {file_name} as {name}'))
