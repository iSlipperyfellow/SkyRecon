...
from core.models import Flight, Detection, Hazard
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for det in detection_objs:
            try:
                ...
                detections.append({
                    ...
                    'bbox': [0,0,0,0], # BBox not currently stored in DB Detection model based on quick look, placeholder
                })
            except Hazard.DoesNotExist:
                continue

        if not detections:
            self.stdout.write(self.style.WARNING(f'No detections found for flight {flight_id}'))
            return
        ...
        if fmt == 'csv':
            ...
            path = generator.generate_markdown_report(detections, inspector_name=f"Flight {flight.mission_name}")

        self.stdout.write(self.style.SUCCESS(f'Successfully generated {fmt.upper()} report at: {path}'))
