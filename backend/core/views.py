...
from core.models import (
    Drone, Flight, Telemetry, Detection, Hazard, Runway,
    InferenceJob, NavigationPlan, User, AIModel
)
from core.serializers import (
    DroneSerializer, DroneCommandSerializer, FlightSerializer,
    TelemetrySerializer, DetectionSerializer, HazardSerializer,
    RunwaySerializer, InferenceJobSerializer, NavigationPlanSerializer,
    HazardEvaluateSerializer, FrameSubmitSerializer, UserSerializer,
    AIModelSerializer
)
...
class DetectionViewSet(viewsets.ModelViewSet):
    ...
    def nearest(self, request):
        ...
        detections = Detection.objects.annotate(
...
class HazardViewSet(viewsets.ModelViewSet):
    ...
    def evaluate(self, request):
        ...
        detection = get_object_or_404(Detection, id=serializer.validated_data['detection_id'])
        runway = None
        
        if 'runway_id' in serializer.validated_data:
            runway = get_object_or_404(Runway, id=serializer.validated_data['runway_id'])
...
class InferenceJobViewSet(viewsets.ViewSet):
    ...
    def submit_frame(self, request):
        ...
        job.save()
        return Response(InferenceJobSerializer(job).data, status=status.HTTP_201_CREATED)
...
class AnalyticsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        label_stats = Detection.objects.values('label').annotate(count=Count('id')).order_by('-count')
        
        # Hazard level distribution
        hazard_stats = Hazard.objects.values('level').annotate(count=Count('id')).order_by('-count')
        
        # Detection trend (last 7 days)
        seven_days_ago = timezone.now() - timezone.timedelta(days=7)
        trend_stats = Detection.objects.filter(timestamp__gte=seven_days_ago) \
...
class SystemHealthView(views.APIView):
    ...
    def get(self, request):
        ...
        return Response({
            'status': 'healthy',
            'uptime': time.clock_gettime(time.CLOCK_BOOTTIME) if hasattr(time, 'CLOCK_BOOTTIME') else 'N/A',
            'infrastructure': {
...
class AIModelViewSet(viewsets.ModelViewSet):
    ...
    def deploy(self, request, pk=None):
        ...
        try:
            ...
            resp = requests.post(inference_url, json={'model_path': model_path}, timeout=10)
            resp.raise_for_status()
            logger.info(f"Inference service reloaded successfully")
            ...
            return Response({
                ...
                'warning': f'Model active in registry but inference service reload failed: {str(e)}',
                'model_id': str(model.id)
            }, status=200) # Still 200 because DB updated
...
class SimulatorControlView(views.APIView):
    ...
    def post(self, request):
        ...
        if not action or not script:
            return Response({'error': 'Missing action or script'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'status': 'Command sent', 'action': action, 'script': script})
        except Exception as e:
